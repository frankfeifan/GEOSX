...
ArrayOfArrays< localIndex > CellBlockManager::getNodeToElements() const
{
  ...
  {
    CellBlock const & cb = this->getCellBlock( blockIndex );
    ...
    forAll< parallelHostPolicy >( cb.numElements(), [elemsPerNode = elemsPerNode.toView(),
                                                     elemToNode, &cb] ( localIndex const cellIndex )
    {
      localIndex const numNodesPerElement = cb.numNodesPerElement();
      for( localIndex a = 0; a < numNodesPerElement; ++a )
      {
        localIndex const nodeIndex = elemToNode( cellIndex, a );
  ...
  for( localIndex blockIndex = 0; blockIndex < numCellBlocks(); ++blockIndex )// Do not need index
  {
    CellBlock const & cb = this->getCellBlock( blockIndex );
    arrayView2d< localIndex const, cells::NODE_MAP_USD > const elemToNode = cb.getElemToNode();
    forAll< parallelHostPolicy >( cb.numElements(), [result = result.toView(),
                                                     elemToNode, &cb] ( localIndex const cellIndex )
    {
      for( localIndex a = 0; a < cb.numNodesPerElement(); ++a )
      {
        localIndex const nodeIndex = elemToNode( cellIndex, a );
        result.emplaceBackAtomic< parallelHostAtomic >( nodeIndex, cellIndex );
      }
    } );
  }
                       ...
                       arrayView2d< localIndex > const & faceToElements )
{
  ...
  GEOSX_ERROR_IF_NE( faceToElements.size( 0 ), numUniqueFaces );
  GEOSX_ERROR_IF_NE( faceToElements.size( 1 ), 2 );
                                           ...
                                           &cellBlocks]( localIndex const nodeIndex )
  {
    ...
    forEqualRanges( faces.begin(), faces.end(), [&]( auto first, auto last )
    {
      ...
      faceToElements( curFaceID, 0 ) = f0.cellIndex;
      faceToElements( curFaceID, 1 ) = -1;

      if( ++first != last )
      {
        NodesAndElementOfFace const & f1 = *first++;
        faceToElements( curFaceID, 1 ) = f1.cellIndex;
      }
...
void CellBlockManager::buildNodeToEdges()
{
  ArrayOfArrays< localIndex > nodeToEdges =
    meshMapUtilities::transposeIndexMap< parallelHostPolicy >( m_edgeToNodes.toViewConst(),
                                                               m_numNodes,
                                                               edgeMapExtraSpacePerNode() );
  m_nodeToEdges.assimilate< parallelHostPolicy >( std::move( nodeToEdges ),
...
{
  ArrayOfArrays< localIndex > nodesToFaces =
    meshMapUtilities::transposeIndexMap< parallelHostPolicy >( m_faceToNodes.toViewConst(),
                                                               m_numNodes,
                                                               faceMapExtraSpacePerNode() );
  ArrayOfSets< localIndex > result;
  result.assimilate< parallelHostPolicy >( std::move( nodesToFaces ),
                                           LvArray::sortedArrayManipulation::UNSORTED_NO_DUPLICATES );
  return result;
}
